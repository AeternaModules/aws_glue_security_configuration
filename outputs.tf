...
output "glue_security_configurations_encryption_configuration" {
  ...
  value       = { for k, v in aws_glue_security_configuration.glue_security_configurations : k => v.encryption_configuration if v.encryption_configuration != null && length(v.encryption_configuration) > 0 }
}
